...
plotVolcEstimate <- function(DE_table,Title){
  EnhancedVolcano(DE_table,lab=NA,x = 'estimate', y = 'q_value',xlab = 'estimate', ylab = 'q_value',xlim = c(-2, 2),title = Title, pCutoff = 0.05,FCcutoff = 0.25,selectLab=rownames(DE_table),pointSize = 0,labSize = 5,maxoverlapsConnectors = 500,colAlpha = 1,
                ...
                gridlines.minor = FALSE,legendLabels=c('Not sig.','Estimate','q<0.05','q<0.05 & β>0.25'),)+
...
for (pat in colnames(FL)){
  ...
  topGenes <- c(topGenes,names(ena[order(ena,decreasing=TRUE)])[1:5])
}
...
for (pat in colnames(MFL)){
  ...
  topGenes <- c(topGenes,names(ena[order(ena,decreasing=TRUE)])[1:20])
}
topGenes <- unique(topGenes)
...
p13 <-plotFGSEAbarcode("bulk_FullDE",toupper(chipseq$ï..ENT_not_DMSO_PePr_peaks_utr5p),"ENT_not_DMSO_PePr_peaks_utr5p")
p10+p11+p12+p13
...
pval13 <-plotLimmaBarcode("bulk_FullDE",toupper(chipseq$ï..ENT_not_DMSO_PePr_peaks_utr5p),"ENT_not_DMSO_PePr_peaks_utr5p")
```
